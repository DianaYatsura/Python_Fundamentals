""" Given a list of numbers and a value n, write a function that returns the probability of
choosing a number greater than or equal to n from the list. The probability should be expressed
as a percentage, rounded to one decimal place.
Notes. Precent probability of event = 100 * (num of favourable outcomes) / (total num of possible outcomes)"""

def probability(lst, num):
    total_outcomes = len(lst)
    favourable_outcomes = 0
    for el in lst:
        if el >= num:
            favourable_outcomes += 1
    probability_of_choosing = 100 * (favourable_outcomes / total_outcomes)
    return round(probability_of_choosing, 1)

print(probability([5, 1, 8, 9], 6))            # 50.0
print(probability([7, 4, 17, 14, 12, 3], 16))  # 16.7